...
export function DoctorProfile({ showShare = false, asPage = false }: DoctorProfileProps) {
  ...
  return (
    <section id={asPage ? undefined : "doctor"} className="bg-gradient-to-b from-brand-soft/40 to-white py-14 sm:py-20">
      <Container>
        {!asPage && (
        ...
        <div className={cn("grid gap-10 lg:grid-cols-12 lg:gap-12", asPage ? "mt-8" : "mt-12")}>
          <div className="lg:col-span-4">
            <div className="card-premium mx-auto max-w-sm overflow-hidden ring-1 ring-brand/10 lg:mx-0">
              <div className="relative aspect-[3/4] bg-gradient-to-br from-brand-soft to-white">
                <Image
                  src={DOCTOR_PORTRAIT}
                  alt={t("name")}
                  fill
              ...
              <div className="border-t border-brand/10 bg-gradient-to-r from-brand-soft to-white p-4 text-center">
                <p className="text-lg font-semibold text-slate-900">{t("name")}</p>
                <p className="text-sm font-medium text-brand">{t("title")}</p>
              </div>
            </div>
            <p className="mt-4 rounded-xl border border-accent/30 bg-accent-soft p-4 text-center text-sm text-slate-700">
              {t("videosComingSoon")}
            </p>
          </div>
...
function Block({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-2xl border border-[var(--border)] bg-white/80 p-5 shadow-sm ring-1 ring-brand/5 sm:p-6">
