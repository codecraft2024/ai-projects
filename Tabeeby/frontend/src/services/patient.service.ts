import { ApiError, apiFetch, apiJson, fileDownloadUrl } from "@/services/api.client";
import type {
  CaseStatus,
  MedicalFile,
  MedicalFileType,
...
export async function uploadPatientFile(
  ...
  const res = await apiFetch(`/api/patients/${id}/files`, {
    method: "POST",
    body: formData,
  });
  ...
  if (!res.ok) {
    ...
    if (res.status === 401) {
      throw new ApiError(401, body?.message ?? "Session expired. Please sign in again.");
    }
