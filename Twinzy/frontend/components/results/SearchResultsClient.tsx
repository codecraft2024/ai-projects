...
async function fetchSession(sessionId: string): Promise<SearchResponse & { sessionId: string }> {
  ...
  if (!response.ok) throw new Error("Search session not found");
...
export default function SearchResultsClient() {
  ...
  if (!sessionId && !localData) {
    return (
      <section className="mx-auto max-w-3xl px-4 py-20 text-center sm:px-6">
        <h1 className="display-title text-3xl">{t("resultsEmpty")}</h1>
        <p className="mt-4 text-muted-foreground">{t("resultsEmptyHint")}</p>
        <Button className="mt-8" onClick={() => (window.location.href = "/scan")}>
          {t("resultsStart")}
        </Button>
      </section>
    );
  ...
  if (!data || !topMatch) {
    return (
      <section className="mx-auto max-w-3xl px-4 py-20 text-center sm:px-6">
        <h1 className="display-title text-3xl">{t("resultsExpired")}</h1>
        <p className="mt-4 text-muted-foreground">{t("resultsExpiredHint")}</p>
        <Link href="/scan" className="mt-8 inline-flex h-10 items-center rounded-full bg-primary px-4 font-bold text-primary-foreground">
          {t("resultsUpload")}
        </Link>
      </section>
    );
  ...
  return (
    <section className="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:space-y-12 sm:px-6 sm:py-12">
      <div>
        <h1 className="display-title text-2xl sm:text-3xl">{t("resultsTitle")}</h1>
        <p className="mt-2 text-sm text-muted-foreground sm:text-base">{t("resultsSubtitle")}</p>
      </div>
      ...
      <div className="space-y-6">
        <h2 className="text-2xl font-bold">{t("humanMatchesTitle")}</h2>
        <HumanMatchGrid
